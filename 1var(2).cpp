...
// Обратная матрица (Гаусса-Жордана)
...
// Находит оптимальные веса для касательного портфеля
vector<double> tangencyPortfolio(const vector<double>& mu, const vector<vector<double>>& cov, double rf) {
    ...
    for (int i = 0; i < n; i++) weights[i] = temp[i] / sumTemp;

    return weights;
}

// Построение эффективной границы (если требуется)
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        cout << "Актив " << (i + 1) << ": ";
        cin >> mu[i];
        while (mu[i] < -1.0 || mu[i] > 10.0) {
            cout << "Ошибка. Повторите: "; cin >> mu[i];
        }
    }

    cout << "Ковариационная матрица " << n << "x" << n << ":\n";
    ...
    cout << "Безрисковая ставка (от 0 до 1): ";
    double rf;
    cin >> rf;
    while (rf < 0.0 || rf > 1.0) {
        cout << "Ошибка. Повторите: "; cin >> rf;
    }

    vector<double> weights;
    try {
        weights = tangencyPortfolio(mu, cov, rf);
    } catch(const std::logic_error& e) {
        cout << "Ошибка: " << e.what() << "\n";
        return 1;
    }

    double portRet = dot(weights, mu);
    double portRisk = portfolioRisk(weights, cov);
    ...
    for (size_t i = 0; i < std::min(size_t(5), eff.size()); ++i)
        cout << eff[i][0] * 100 << "\t\t" << eff[i][1] * 100 << "\n";
    if (eff.size() > 5)
        cout << "... (всего " << eff.size() << " точек)\n";
    return 0;
}
